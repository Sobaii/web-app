...
import fetchCallWrapper from "../services/fetchCallWrapper";


const fetchSpreadsheetData = () => {
  return fetchCallWrapper('http://localhost:3000/users/spreadsheets-shallow-info')
    .then(res => {
      if (!res.ok) throw new Error('Failed to fetch spreadsheet data')
      return res.json()
    })
};
const spreadsheedResource = createResource(fetchSpreadsheetData())
...
function DashboardDumm() {
  ...
  const spreadsheets = spreadsheedResource.read()
